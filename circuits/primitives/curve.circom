...
template PointOnCurve(n, k, a, b, p){
    ...
    component cu_red = PrimeReduce(n, k, 2*k-2, p, 4*n + 3*LOGK + 1); //log((2k^3-k^2)*2^4n)
...
// Implements:
// out = x^3 + b mod p  
template BN254Function(n, k, b, p) {
    signal input in[k];
    signal output out[k];
    ...
    // x_cu + a x + b has 3k-2 positive registers < k^2 * 2^{3n} + 2^n < (k^2 + 1) * 2^{3n} 
    component cu_red = PrimeReduce(n, k, 2*k-2, p, 4*n + 3*LOGK + 1); //log((2k^3-k^2)*2^4n)
    for(var i=0; i<3*k-2; i++){
        if(i == 0)
            cu_red.in[i] <== x_cu.out[i] + b; 
        else{
            cu_red.in[i] <== x_cu.out[i];
        }
    }

    component big_mod = SignedFpCarryModP(n, k, 4*n + 3*LOGK + 1, p);
    for (var i = 0; i < k; i++)
        big_mod.in[i] <== cu_red.out[i];
