...
include "./helpers/bigint_func.circom";
...
// split[0][0], split[0][1] * 2^n,split[0][2] * 2^2n, 
// split[1][0]*2^n, split[1][1]*2^2n, split[1][2]*2^3n,
// split[2][0]*2^2n, split[2][1]*2^3n, split[2][2]*2^4n,
    var carry[k];
    carry[0] = 0;
    out[0] <-- split[0][0];
    if (k > 1) {
...
template PrimeReduce(n, k, m, p, m_out){
    ...
    e[0] = n;
    var pow2n[50] = mod_exp(n, k, two, p, e); // 2 ^ n
    e[0] = k;
    assert(k < (1<<n) );
    var pow2nk[50] = mod_exp(n, k, pow2n, p, e); // 2 ^ (k*n)
