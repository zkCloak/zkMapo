...
include "./helpers/bn254_func.circom";
...
template OptimalAtePairingMulti(m, n, k, p){
    signal input P[m][2][2][k]; 
    signal input Q[m][2][k];
    signal output out[6][2][k];

    var bn254_b[2][50] = get_bn254_b(n, k);

    component miller = OptimizedMillerLoopProductFp2(n, k, bn254_b[0], bn254_b[1], m, get_bn254_pseudo_binary_encoding(), 65, p);
    for(var l=0;l<m;l++)
        for(var i=0; i<2; i++)
            for(var j=0; j<2; j++)
                for(var idx=0; idx<k; idx++)
                    miller.P[l][i][j][idx] <== P[l][i][j][idx];
    for(var l=0;l<m;l++)
        for(var i=0; i<2; i++)
            for(var idx=0; idx<k; idx++)
                miller.Q[l][i][idx] <== Q[l][i][idx];
